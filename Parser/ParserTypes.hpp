...
struct PrimitiveTypeNode : Type {
  PrimitiveType type;
  explicit PrimitiveTypeNode(const PrimitiveType t) : type(t) {}
...
struct FunType : Type {
  ...
  [[nodiscard]] std::unique_ptr<Type> clone() const override {
    ...
    return std::make_unique<FunType>(std::move(paramTypesCopy), returnType->clone());
  }
};
...
struct PointerType : Type {
  ...
  [[nodiscard]] std::unique_ptr<Type>
  clone() const override {
    return std::make_unique<PointerType>(referencedType->clone());
  }
};
...
struct ArrayType : Type {
  ...
  int size;
  ArrayType(std::unique_ptr<Type> elemType, const int s) : elementType(std::move(elemType)), size(s) {}
